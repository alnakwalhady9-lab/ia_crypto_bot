...
def get_market_candles(granularity=900, limit=100):
    ...
    try:
        r = requests.get(url, params={"granularity": granularity}, timeout=10, headers={"User-Agent": "ia-crypto-bot/1.3", "Accept": "application/json"})
        r.raise_for_status()
        rows = r.json()
...
def build_4h_candles(hourly):
    """Aggregate Coinbase 1h candles into UTC-aligned 4h candles."""
    if not hourly:
        return None
    buckets = {}
    for row in hourly:
    ...
    for bucket in sorted(buckets):
        ...
        if len(group) != 4:
            continue
        low = min(float(x[1]) for x in group)
        high = max(float(x[2]) for x in group)
        open_price = float(group[0][3])
        close_price = float(group[-1][4])
        volume = sum(float(x[5]) for x in group)
        result.append([bucket, low, high, open_price, close_price, volume])
    return result
...
def atr(candles, period=14):
    ...
    for i in range(1, len(candles)):
        high, low, prev_close = float(candles[i][2]), float(candles[i][1]), float(candles[i-1][4])
...
def get_news_sentiment():
    ...
    for url in NEWS_FEEDS:
        try:
            r=requests.get(url,timeout=10,headers={"User-Agent":"Mozilla/5.0 ia-crypto-bot/1.3"}); r.raise_for_status(); root=ET.fromstring(r.content)
    ...
    headlines=list(dict.fromkeys(headlines))[:20]; label,score=score_texts(headlines)
...
def fetch_reddit_sentiment():
    ...
    for url in REDDIT_FEEDS:
        try:
            r=requests.get(url,timeout=10,headers={"User-Agent":"ia-crypto-bot/1.3 by market-research-bot","Accept":"application/atom+xml,application/rss+xml"})
    ...
    posts=list(dict.fromkeys(posts))[:20]; label,score=score_texts(posts)
...
def analyze_market():
    c15=get_market_candles(900,100)
    # Request enough 1h history to construct at least 51 complete 4h candles.
    c1h=get_market_candles(3600,300)
    c4h=build_4h_candles(c1h)
    ...
    closed15=c15[:-1]; volumes=[float(c[5]) for c in closed15]; price,rsi14=s15["price"],s15["rsi"]
    avg_volume=sum(volumes[-21:-1])/20; volume_ratio=volumes[-1]/avg_volume if avg_volume else 0; atr14=atr(closed15,14)
    recent=closed15[-20:]; support=min(float(c[1]) for c in recent); resistance=max(float(c[2]) for c in recent)
    ...
    if t15=="BULLISH" and rsi14 is not None and 52<=rsi14<=70 and volume_ratio>=1.05:
        technical_signal,technical_points="LONG",60; reasons.extend(["15m bullish EMA structure","Bullish RSI momentum","Volume confirmation"])
    elif t15=="BEARISH" and rsi14 is not None and 30<=rsi14<=48 and volume_ratio>=1.05:
        technical_signal,technical_points="SHORT",60; reasons.extend(["15m bearish EMA structure","Bearish RSI momentum","Volume confirmation"])
    ...
    news,reddit=get_news_sentiment(),get_reddit_sentiment(); confidence=technical_points
    if technical_signal=="LONG":
