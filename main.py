...
def get_market_candles(granularity=900,limit=100):
    try:
        r=requests.get("https://api.exchange.coinbase.com/products/BTC-USD/candles",params={"granularity":granularity},timeout=10,headers={"User-Agent":"ia-crypto-bot/1.4","Accept":"application/json"}); r.raise_for_status(); rows=r.json()
...
def get_news_sentiment():
    ...
    for url in NEWS_FEEDS:
        try:
            r=requests.get(url,timeout=10,headers={"User-Agent":"Mozilla/5.0 ia-crypto-bot/1.4"});r.raise_for_status();root=ET.fromstring(r.content)
...
def fetch_reddit_sentiment():
    ...
    for url in REDDIT_FEEDS:
        try:
            r=requests.get(url,timeout=10,headers={"User-Agent":"ia-crypto-bot/1.4 by market-research-bot","Accept":"application/atom+xml,application/rss+xml"})
...
while True:
    ...
    if a:
        ...
        if signal in ("LONG","SHORT") and signal!=last_signal:
            msg=(f'BTC/USD SIGNAL: {signal}\nEntry reference: {a["price"]:.2f}\nStop Loss: {a["stop_loss"]:.2f}\nTake Profit: {a["take_profit"]:.2f}\nConfidence: {a["confidence"]}%\nRSI: {a["rsi"]:.1f}\nATR: {a["atr"]:.2f}\nSupport/Resistance: {a["support"]:.2f} / {a["resistance"]:.2f}\nTrends 15m/1h/4h: {a["trends"]["15m"]}/{a["trends"]["1h"]}/{a["trends"]["4h"]}\nWhy: {"; ".join(a["reasons"])}\n\nAnalysis alert only - no trade executed.');send_telegram_message(msg)
